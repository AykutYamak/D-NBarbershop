    ...
    public class MessageModelTests
    {
        ...
        "invalid@.com"
            ...
            foreach (var invalidEmail in invalidEmails)
            {
                ...
                Assert.That(isValid.Equals(false) || isValid == false, $"Message with invalid email '{invalidEmail}' should fail validation");

                Assert.That("Вашият e-mail трябва да съдържа валиден домейн и поне една точка, както и символ '@'!" == validationResults[0].ErrorMessage || "Невалиден e-mail формат." == validationResults[0].ErrorMessage, $"Unexpected error message for email '{invalidEmail}'");
            }
        }
